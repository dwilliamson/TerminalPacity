...
void copystring(byte *str_dest, byte *str_src)
{
   word x = 0;
                      ...
                      byte wmy)
{
   ...
   if (x == numslots && x != 0)
   {
      if (x == 0)
      {
         whandle->XPos       = _fmalloc(sizeof(byte));
         whandle->YPos       = _fmalloc(sizeof(byte));
         whandle->Width      = _fmalloc(sizeof(byte));
         whandle->Height     = _fmalloc(sizeof(byte));
         whandle->Type       = _fmalloc(sizeof(byte));
         whandle->Used       = _fmalloc(sizeof(bool));
         whandle->Hidden     = _fmalloc(sizeof(bool));
         whandle->Title      = _fmalloc(sizeof(byte far *));
         whandle->InfoString = _fmalloc(sizeof(byte far *));

         if (wtype == WT_HEADONLY || wtype == WT_SINGLE)
         {
            whandle->NumStrings    = _fmalloc(sizeof(byte));
         }
      }
   }
