...
void Move_Enemies(void)
{
   ...
   for (x = 0; x < num_mons; x++)
   {
      ...
      if (monpos_store[mod] == 4)
      {
         Power_Erase_16((monpos_store[mod + 1] << 8) + monpos_store[mod + 2], monpos_store[mod + 3]);
         monpos_store[mod + 3] -= alpha_table[alpha];
         cx = (monpos_store[mod + 1] << 8) + monpos_store[mod + 2];
         cx += alpha_table[alpha];
         monpos_store[mod + 1] = (cx >> 8) & 255;
         monpos_store[mod + 2] = cx & 255;
      }
      if (monpos_store[mod] == 5)
      {
         mx = (monpos_store[mod + 1] << 8) + monpos_store[mod + 2];
         dx = pacman.x - mx;
         if (dx < 16 && dx > -16 && pacman.y < monpos_store[mod + 3])
         {
            offset = pixtab[monpos_store[mod + 3] + 40] + mx;
            if (video_buffer[offset + 5440] != 155)
               monpos_store[mod + 3] -= 2;
         }
         if (dx < 16 && dx > -16 && pacman.y > monpos_store[mod + 3])
         {
            offset = pixtab[monpos_store[mod + 3] + 40] + mx;
...
//         if (level_store[temp] >= 64 && level_store[temp] < 68)
         if (level_store[temp] >= 64 && level_store[temp] != 68 && level_store[temp] != 69)
         {
            multemp = (num_mons << 3) + num_mons;
            if (level_store[temp] < 68)
               monpos_store[multemp]   = level_store[temp] - 64;
            else monpos_store[multemp] = level_store[temp] - 66;
...
void Initialise_Sprites(void)
{
   ...
   // change2
   Sprite_Init((sprite_ptr)&enemy, 100, 100, 0, 0, 0, 0, 16, 16, 19);
...
void Get_Sprite_Info(void)
{
   ...
   // change3
   for (x = 0; x < 6; x++)
      Grab_Bitmap((sprite_ptr)&enemy, x + 12, x, 8);
   for (x = 5; x < 8; x++)
      Grab_Bitmap((sprite_ptr)&key_stuff, x - 5, x, 4);
   // change4
   Grab_Bitmap((sprite_ptr)&enemy, 18, 11, 2);
}
...
void Do_Cheat(void)
{
   static unsigned char curvar = 0, itsoff[5] = { 0, 0, 0, 0, 0 };

   if (key_table[11])
   {
      itsoff[0] = 1;
   }
   if (!key_table[11] && itsoff[0])
   {
      itsoff[0] = 0;
      curvar = 1;
   }
...
void main(int argc, char *argv[])
{
   ...
   Set_Video_Mode(0x03);
   Change_Colour();
   ptext(" Terminal Pacity v1.50                        Copyright (c) 1997 Don Williamson ", 0, 0, 30);
   printf("\n");
   if (argc > 1)
   {
